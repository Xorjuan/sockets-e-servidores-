...
# Decorator para operações atômicas
...
# Contador atômico para conexões
class AtomicCounter:
    def __init__(self):
        self._value = 0
        self._lock = Lock()
...
# Inicialização do contador de conexões
connection_counter = AtomicCounter()

# ASCII matrix e UTF-8 table
ascii_matrix = [
...
# Funções de conversão e operação
@atomic_operation
...
def handle_client(conn, addr):
    connection_id = connection_counter.increment()
    logging.info(f"Nova conexão {connection_id} de {addr}")
    try:
        with conn:
            while True:
        ...
        logging.info(f"Conexão {connection_id} de {addr} encerrada")

def start_server(host='127.0.0.1', port=65432, max_connections=10):
